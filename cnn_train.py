...
class CNN_Relation(object):
	...
	def __init__(self, num_classes, seq_len, word_dict_size, pos_dict_size, chunk_dict_size, dep_dict_size, d1_dict_size, d2_dict_size, type_dict_size, batch_size = 50, w_emb_size=50, d1_emb_size=5, d2_emb_size=5, pos_emb_size=5, chunk_emb_size=5, dep_emb_size=5, type_emb_size=5, filter_sizes=[2,3,5], num_filters=70, l2_reg_lambda = 0.0):

		emb_size = w_emb_size + pos_emb_size + chunk_emb_size + d1_emb_size + d2_emb_size + type_emb_size + dep_emb_size
		...
		self.dep = tf.placeholder(tf.int32, [None, seq_len], name='x6')
		...
		W_depemb =  tf.Variable(tf.random_uniform([dep_dict_size, dep_emb_size], -1.0, +1.0))
		...
		emb6 = tf.nn.embedding_lookup(W_depemb, self.dep)				#POS embedding

		X = tf.concat(2, [emb0, emb1, emb2, emb3, emb4, emb5, emb6])		#shape(?, 21, 100)
	...
	def train_step(self, W_batch, pos_batch, chunk_batch, d1_batch, d2_batch, t_batch, dep_batch, y_batch):
    		feed_dict = {
				self.w 		:W_batch,
				self.pos	:pos_batch, 
	...
	def test_step(self, W_batch, pos_batch, chunk_batch, d1_batch, d2_batch, t_batch, dep_batch, y_batch):
    		feed_dict = {
				self.w 		:W_batch,
				self.pos	:pos_batch, 
				self.chunk	:chunk_batch,
